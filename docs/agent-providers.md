...
connecting to servers, fails closed if inventory fails, and explicitly disables
...
Inventory is bounded to ten seconds. Unsupported server names/transports fail
closed. Only server names and transport types are persisted in each Codex
...
flags used here. SDK 0.1.2 carries kind/args without a local path dependency.
...
checkout alone does not change the running factory. No service restart,
operational prompt, Jira change, commit, or publication is part of validation.
