  ...
  return {
    ...
    stand_down: {
      description:
        ...
        'It re-reads, directly from Jira, the current comment ids on your own ticket and on every ticket you currently have a worker on, and records that as what you have already seen. WHAT WAKES YOU, EXACTLY (characterized by test, not aspirational): a STATUS change on any of those tickets (a worker reaching In Review is the common one) always wakes you; a SUMMARY edit on any of them always wakes you; a pr:* review-state transition on YOUR OWN ticket always wakes you; and a NEW COMMENT that is not in the recorded set — on your own ticket or a worker\'s, including a question, a report, or a [butchr:blocked] escalation — wakes you. WHAT DOES NOT WAKE YOU: a daemon label change on a WORKER\'s ticket on its own — that worker\'s agent:working/agent:idle flips, and a pr:* transition on the WORKER\'s ticket unaccompanied by a new comment. That is deliberate (those flip constantly and would wake a boss for nothing), so do not stand down expecting to be woken by a worker\'s PR label moving; the events you actually wait for (a worker reaching In Review, a worker asking you something, an escalation) are all in the waking list. ' +
