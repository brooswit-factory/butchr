  ...
  return {
    ...
    get_doc: {
      description:
        ...
        'PAGINATION: you are finished reading when `next` is ABSENT — never infer completion from `complete`, which stays `false` on every slice of a partial read, including the very last one (it still isn\'t the whole document in one result). To read a large document in full: call with no `offset` (or `offset: 0`), then keep calling again with `offset` set to the PREVIOUS result\'s `next.offset`, concatenating each `chunk` in order, until a result has no `next`. `version` is the page\'s own Confluence version number (free on this read) — compare it across calls if you need to notice the document changed underneath a multi-call read.',
      input: { key: z.string().optional(), offset: z.number().int().optional(), limit: z.number().int().optional() },
      handler: async (a, c) => {
        const { key, offset, limit } = a as { key?: string; offset?: number; limit?: number };
        const who = c.headers["x-issue"];
        ...
        return isProjectId(target) ? getProjectDoc(ops, target, offset, limit) : getDoc(ops, target, offset, limit);
      },
    },
    set_doc: {
      description:
