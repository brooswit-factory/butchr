  ...
  const tools: Record<string, ToolDef<any>> = {
    ...
    adopt_worker: {
      ...
      handler: async (a, c) => {
        ...
        // BUTCHR-151/BUTCHR-157: same audit-line pattern as the identity
        // fields above — a distinct greppable line for both the success and
        // the "found it but could not retire it" case, never silent either way.
        if (result.orphanHeaderWithdrawn) audit(c, `adopt_worker ${result.key} ORPHAN HEADER WITHDRAWN: ${result.orphanHeaderWithdrawn}`);
        if (result.orphanHeaderNotWithdrawn) audit(c, `adopt_worker ${result.key} ORPHAN HEADER NOT WITHDRAWN: ${result.orphanHeaderNotWithdrawn}`);
        noted(c, [p.key, who]);
        return result;
      },
    },
    ...
    correct_worker: {
      description:
        ...
        "TWO LIMITATIONS, READ BEFORE YOU CALL THIS: (1) ownership at the epic tier is exactly what `assertOwnWorker` checks, not a matter of which tiers happen to exist: a PROJECT caller may correct a target that is BOTH a member of its own project AND an Epic, so a project agent CAN correct one of its own epics' descriptions with this verb. That does not relax the self-refusal above — an epic can still never correct its OWN description with this verb, at any tier, project callers included; nobody corrects themselves. Where `assertOwnWorker` accepts no caller for a given ticket, the recourse is a person editing it directly in the Jira UI, the same \"the human is the fallback, not the first responder\" arrangement this fleet already runs on elsewhere. (2) a `description` is read LIVE — every `jira_get_issue` reaches it immediately, including an agent that spawns later — but a `summary` is SNAPSHOTTED into a workspace's `brief.md` ONLY (never `CLAUDE.md`) at build time: correcting it updates Jira, the board and every future read, AND (BUTCHR-169) best-effort regenerates `brief.md` for any workspace already on disk for that worker (reported in the result's `summaryWorkspaceRewrite`) — but this can NEVER reach a worker's agent that is CURRENTLY RUNNING, because no file rewrite can edit a running process's memory; follow up with `tell_worker` if that agent needs to know. " +
    ...
    finish_without_a_boss: {
      description:
        "Move the CALLER'S OWN ticket to Done — but ONLY when the caller has NO BOSS at all (today, in practice, an Epic in a project that carries no project-tier root doc). NOT YOUR DEFAULT CLOSING STEP, AND NOT THE SAFE CHOICE WHEN UNSURE — submit_to_boss is (see its own description); call this only when you KNOW your project has no project tier. TAKES NO ARGUMENTS AT ALL, same reasoning as submit_to_boss: the only ticket this can ever act on is the caller's own, so there is nothing to get wrong. " +
        "REFUSES any caller that HAS a boss — that refusal IS this verb's entire purpose, not a guard bolted onto it: a worker never finishes itself (see finish_worker), and a caller with a boss already has the review hop that guarantee depends on waiting for it — submit_to_boss, then that boss's own finish_worker. \"Has a boss\" now means EITHER of two things (BUTCHR-92/BUTCHR-326): an `Implements` inward link (checked first, no extra Jira call), OR being an Epic whose own project carries a project-tier `butchr` entity property naming a root doc (such a project reviews its own Epics exactly as any other boss reviews its workers) — either refusal names the boss (the linked ticket, or the project key) and points at submit_to_boss instead, and says why: every Done in this system requires a second identity to have looked at the work first, except this one deliberate, narrow exception for a ticket that truly has nobody to submit to and nobody who will ever call finish_worker on it. THE PROJECT HALF IS A DIRECT, CREDENTIAL-INVARIANT READ of that one project's own property (BUTCHR-333) — never the lead-filtered `resolveEligibleProjects` search, whose verdict would otherwise depend on which daemon's credential happened to handle this call. " +
        "FAILS CLOSED on the project-tier check: if reading the project's property errors out, this refuses with a message that names the underlying error rather than ever treating \"could not look\" as \"no boss\". A property-carrying project that no running daemon currently leads still refuses the same way — the Epic then waits, visibly, In Review, with nobody around yet to call finish_worker; that visible wait is the intended, safe outcome, not a bug. " +
        "A bossless Story or Task that merely SHARES a project with an Epic is NOT caught by the project-tier half — a project boss's own workers are its Epics only (same as assertOwnWorker's own project-caller check), so gating this on issuetype is what keeps a refusal here from ever pointing a ticket at a submit_to_boss/finish_worker path with no exit. That is a known, accepted narrow gap, not an oversight. " +
