...
export const MEDIA_REGISTRY: Readonly<Record<Medium, MediaRegistryEntry>> = {
  ...
  docTitle: {
    ...
    noBlindSpotIdsReason:
      "docTitle has no scanner module (src/media/media-scan.ts's own header names this absence) and therefore no blind-spot id list of its own to tie to — inventing one here would misrepresent a medium that is genuinely, deliberately not enforced by a source-scanning list at all. This is a DIFFERENT claim from media-scan.ts's own MEDIA_SCAN_BLIND_SPOTS entries (those are about media-scan's blindness to a structurally-enforced medium's shape, and about whether the hand-maintained set of such media is current; this one is about docTitle having no list of its OWN residual gaps to reference) — see blindSpots below for what registry.ts can honestly say about docTitle's own gaps instead.",
    blindSpots:
