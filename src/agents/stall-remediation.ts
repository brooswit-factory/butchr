...
function wakeComment(issue: string, elapsedMinutes: number): string {
  return [
    `${MARKER} ${issue} has read agent:stalled, continuously, for ${elapsedMinutes} minute(s): idle or done since it last stopped working, with no comment from this daemon's account in that window.`,
    "",
    `fingerprint: ${issue}`,
    "",
