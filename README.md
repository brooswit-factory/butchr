...
Atlassian is optional (FACTORY-66): leave `ATLASSIAN_SITE`/`ATLASSIAN_EMAIL`/`ATLASSIAN_TOKEN`(`_FILE`) unset entirely to run a managed-sessions-only host with Jira/Confluence disabled — the daemon starts, logs why, and serves managed sessions/github-issue/zendesk-ticket/filesystem rules exactly as before. Set all three, or none; setting only one or two is a startup error. It refuses to start if a loaded `jira-work`/`jira-idea`/`jira-project` rule is enabled with no Atlassian configured, since that rule could never be staffed. See `.env.example`'s own comment for the exact presence rule and what stays off.
