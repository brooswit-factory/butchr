...
**Capacity naming.** `docs/rules.example.json` is written against the rule schema on `main` as of this writing, which has no capacity-role field. BUTCHR-392 is expected to add a `workers`/`sentinels` capacity naming to `Rule` (`src/rules/rules.ts`); once that lands, the example and this section should adopt it. Until then, don't invent the field — every rule above runs as today's only mode (one agent per matching ticket).
