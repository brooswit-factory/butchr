...
**Convention: every ticket-worker (`jira-work`) rule is staffed only `In Progress` / `In Review`; `To Do` and `Done` are excluded BY THE QUERY, not by any code-level gate.** A query like `assignee = currentUser() AND issuetype = Task AND statusCategory != Done` also matches `To Do` — it takes a capacity slot for a ticket nobody is meant to be working yet, and a ticket *moved to* `To Do` wakes that agent instead of stopping it. Write `status IN ("In Progress", "In Review")` instead. [`docs/rules.example.json`](docs/rules.example.json) is the canonical starting point: one `jira-work` rule per role (`epics`, `stories`, `tasks`, `bugs`, `subtasks`), each `assignee = currentUser()` plus an `issuetype` clause plus `status IN ("In Progress", "In Review")`. It loads through the real loader/validator in `test/unit/rules-example.test.ts`, which also asserts every rule in it keeps this shape — copy it as your rules file's starting point and add provider rules (`github-issue`, `jira-idea`, `zendesk-ticket`) or narrow the JQL further as needed. There is no code-level status gate in `jira-work` staffing and none is planned — the query is the only place a status floor lives, so it stays fully operator-configurable; an enabled `jira-work` rule whose JQL has no `status` clause at all only gets a startup `WARNING:` line (never a refusal, never an implicit gate — see `src/rules/status-clause-warning.ts`).

**Capacity naming.** Every rule above carries no capacity flag: the default capacity role is `worker`, so the common case — a ticket-worker rule — needs no field at all. Only an always-on agent is ever flagged `sentinel`. The concrete field for this has not landed on `main` as of this writing; see BUTCHR-392 (`src/rules/rules.ts`) once it has, and name it here exactly as the schema does rather than guessing ahead of it.
