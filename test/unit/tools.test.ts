...
function rig(roles: { story?: string; task?: string } = {}) {
  ...
  const ops: AtlassianOps = {
    ...
    assign: rec("assign"), createPage: rec("createPage"), getPage: rec("getPage"), updatePage: rec("updatePage", { version: 1 }) as unknown as AtlassianOps["updatePage"], searchPages: rec("searchPages", { results: [] }), listSpaces: rec("listSpaces"),
...
describe("jira_link_issues invalid MCP result (KAN-764)", () => {
  test("substitutes a real value when the op resolves undefined (Jira's empty 201 body)", async () => {
    ...
    const ops: AtlassianOps = {
      ...
      createPage: async () => ({}), getPage: async () => ({}), updatePage: async () => ({ version: 1 }), searchPages: async () => ({}), listSpaces: async () => ({}),
      ...fakeDocOps(),
    commentOnPage: async () => ({ ok: true }),
    getPageComments: async () => ({ results: [] }),
    searchProjects: async () => ({ values: [] }),
...
describe("jira_create_issue: role assignment, implements/parent resolution, orphan opt-out (KAN-802)", () => {
  ...
  function rig(customRoles: { story?: string; task?: string } = roles, opsOverrides: Partial<AtlassianOps> = {}) {
    ...
    const ops: AtlassianOps = {
      ...
      assign: rec("assign"), createPage: rec("createPage"), getPage: rec("getPage"), updatePage: rec("updatePage", { version: 1 }) as unknown as AtlassianOps["updatePage"], searchPages: rec("searchPages"), listSpaces: rec("listSpaces"),
      ...fakeDocOps(),
      ...opsOverrides,
    commentOnPage: async () => ({ ok: true }),
    getPageComments: async () => ({ results: [] }),
...
describe("confluence_update_page result normalization", () => {
  test("ops.updatePage resolving undefined still produces a defined MCP result (same orOk shape as the other write tools)", async () => {
    const ops: AtlassianOps = {
      ...
      updatePage: (async () => undefined) as unknown as AtlassianOps["updatePage"], searchPages: async () => ({}), listSpaces: async () => ({}),
      ...fakeDocOps(),
    commentOnPage: async () => ({ ok: true }),
    getPageComments: async () => ({ results: [] }),
    searchProjects: async () => ({ values: [] }),
...
describe("confluence_search_pages", () => {
  function rig(opsOverrides: Partial<AtlassianOps> = {}) {
    ...
    const ops: AtlassianOps = {
      ...
      assign: rec("assign"), createPage: rec("createPage"), getPage: rec("getPage"), updatePage: rec("updatePage", { version: 1 }) as unknown as AtlassianOps["updatePage"],
...
describe("the ten relationship verbs (BUTCHR-35): wiring — x-issue, schema shape, audit, onWrite", () => {
  ...
  test("start_worker / shelve_worker / finish_worker / prioritize_worker / tell_worker audit lines name the verb and the target key", async () => {
    ...
    const ops: AtlassianOps = {
      ...
      createPage: rec("createPage"), getPage: rec("getPage"), updatePage: rec("updatePage", { version: 1 }) as unknown as AtlassianOps["updatePage"], searchPages: rec("searchPages"), listSpaces: rec("listSpaces"),
      ...fakeDocOps(),
    commentOnPage: async () => ({ ok: true }),
    getPageComments: async () => ({ results: [] }),
    searchProjects: async () => ({ values: [] }),
