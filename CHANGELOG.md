...
## [0.5.14] - 2026-08-28
### Fixed
- **The escalation comment could carry command output and credentials that scrolled above the dialog — closed.** KAN-734's `escalationComment()` was built only from `prompt.question` and `prompt.options`, exactly as KAN-732 specified — but `question` itself absorbed every preceding pane line with no bound, so surrounding terminal output (a `cat .env`, a prior command's stdout) rode along into a durable, project-readable Jira comment regardless. Two independent fixes, both required: `parsePrompt`'s numbered branch now shares the same 6-line tail bound the un-numbered branch already had (one constant, `QUESTION_TAIL`, so the two can't drift apart again); and a new exported `redact()` is applied to the question and every option inside `escalationComment()`, masking `KEY=VALUE`/`KEY: VALUE` secrets, provider token shapes (`ghp_…`, `sk-…`, `xox…`, `AKIA…`), URL-embedded credentials, `Authorization: Bearer/Basic` headers, and long opaque blobs — value only, keeping the surrounding shape legible (`AWS_SECRET_ACCESS_KEY=[redacted]`). The assembled question is also capped at 600 characters with a `…[truncated]` marker. The two REAL captured dialogs already in `test/unit/prompt.test.ts` parse and escalate unchanged (they carry no secrets); a pane carrying a credential no longer does.
