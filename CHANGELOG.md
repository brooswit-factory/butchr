...
## [0.5.18] - 2026-08-28
### Fixed
- **Escalation stability, finished.** v0.5.16 was the tourniquet: a per-pane rate cap and a fix so a refused answer re-earns the debounce instead of re-escalating instantly. Neither closed the actual hole. Measured on a live daemon after v0.5.16 shipped: three MORE escalations landed on one ticket, each quoting a WORKING agent's own pane text — the fingerprint was stable across the whole 2-poll debounce window each time, so the debounce fix could not have stopped them. The parser was accepting them as real dialogs. Root cause, confirmed against real captures: `parsePrompt`'s numbered-option branch had no positional discipline — ANY two `N. label` lines anywhere in a pane's scrollback parsed as a menu, including a pane that merely quotes option text out of its own Jira ticket (a self-sustaining loop: butchr escalates, the comment quotes the options, the agent reads its ticket, the pane now contains those same two lines, butchr escalates again — nine comments on one ticket in six minutes was the original incident this whole fix traces back to). `parsePrompt` now requires two independent structural properties before accepting a numbered menu: the confirm/select footer must immediately follow the last option (no unrelated content between them), and exactly one option line must carry the `❯` cursor. A genuine dialog satisfies both; scrollback prose that merely mentions options or a footer satisfies at most one. Two secondary gaps closed alongside it: a blocked pane whose text fails to parse now logs once (deduplicated by content, not once per 5-second poll) instead of vanishing silently — the hardened parser rejects more panes, and a real dialog wrongly rejected must be visible, not stuck; and a poll on which a pane is reported not-blocked (or reported blocked with unparseable text) now correctly resets that pane's debounce, closing a flicker case where a pane blinking blocked/idle/blocked could still reach two "consecutive" observations that were never actually consecutive. Also finished: a stale `ANSWER` comment is now refused at most once ever per pane (previously the refused comment's id was forgotten on every debounce reset and could be re-read and re-refused indefinitely on a pane that stayed blocked); the rate cap is now keyed by pane rather than by issue, since a pane can outlive its issue key in the herd; and an escalation adopted after a daemon restart now correctly counts against that pane's hourly budget instead of granting a free refill.
